...
class CohereScraper(EnhancedBaseScraper):
    ...
    def extract_structured_deprecations(self, html: str) -> List[DeprecationItem]:
        ...
        # Find main content
        main = soup.find('main') or soup.find('article') or soup.find('div', class_='markdown')
        if not main:
            return items
    ...
    def _extract_from_section(
        ...
        if "Command-R-03-2024" in title or "Command-R-03-2024" in content:
            ...
            shutdown_match = re.search(r'(?:until|by|on)\s+(\w+\s+\d{1,2},?\s+\d{4})', content)
            shutdown_date = ""
            if shutdown_match:
            ...
            for model_id, replacement in rerank_models:
                if model_id in content or model_id.replace('-', ' ') in content.lower():
                    item = DeprecationItem(
                        ...
                        model_id=model_id,
                        model_name=model_id,
                        announcement_date=announcement_date or "2024-12-02",  # From example
            ...
            shutdown_match = re.search(r'(?:on|by)\s+(\w+\s+\d{1,2},?\s+\d{4})', content)
    ...
    def _contains_model_deprecation(self, content: str) -> bool:
        ...
        deprecation_keywords = [
            'deprecat', 'sunset', 'retire', 'end-of-life', 'eol',
            'discontinued', 'removed', 'shut down', 'will be unsupported'
        ]
        
        model_keywords = [
            'model', 'endpoint', 'api', 'command', 'embed', 'rerank',
            'classify', 'generate', 'summarize'
        ]
        
        content_lower = content.lower()
        has_deprecation = any(keyword in content_lower for keyword in deprecation_keywords)
