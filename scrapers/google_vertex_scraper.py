...
class GoogleVertexScraper(EnhancedBaseScraper):
    ...
    def extract_structured_deprecations(self, html: str) -> List[DeprecationItem]:
        ...
        for section_header in sections:
            ...
            while next_elem:
                if hasattr(next_elem, 'name'):
                    ...
                    if 'Deprecation date:' in text or 'deprecation date:' in text:
                        ...
                        if dep_match:
                            deprecation_date = self.parse_date(dep_match.group(1).strip())
                        if shut_match:
                    ...
                    if next_elem.name == 'ul':
                        ...
                        for li in next_elem.find_all('li'):
                            ...
                            model_match = re.search(r'([\w-]+@\d+|imagegeneration@\d+)', li_text)
                            if model_match:
                                ...
                                item = DeprecationItem(
                                    ...
                                    model_id=model_id,
                                    model_name=f"Imagen {model_id}" if 'imagen' in model_id else model_name,
                                    announcement_date=deprecation_date,
                                    shutdown_date=shutdown_date,
                                    replacement_model="Imagen 3" if 'imagen' in li_text.lower() else None,
                                ...
                                if feature_name and any(keyword in feature_name.lower() for keyword in ['caption', 'question', 'vqa']):
                                    item = DeprecationItem(
                                        provider=self.provider_name,
            ...
            if (deprecation_date or shutdown_date) and not any(item for item in items if item.deprecation_context == ' '.join(context_parts)):
                # Clean section title
                model_name = section_title
                if 'deprecation' in model_name.lower():
                    model_name = model_name.replace('deprecation', '').replace('deprecations', '').strip()
                    ...
                    if models:
                        ...
                        for model in models:
                            item = DeprecationItem(
                                ...
                                shutdown_date=shutdown_date,
                                replacement_model="Imagen 3" if "imagen 3" in details.lower() else replacement,
