...
class TestStaticSiteGenerator:
        ...
        return FeedData(
            deprecations=[
                DeprecationEntry(
                    ...
                    url="https://openai.com/deprecations"
                ),
                DeprecationEntry(
                    provider="Anthropic",
                    model="claude-2",
    ...
    def test_generate_site(self, mock_file, mock_copy, mock_mkdir, sample_feed_data: FeedData):
        ...
        generator.env.get_template = MagicMock(return_value=MagicMock(
            render=MagicMock(return_value="<html>Rendered HTML</html>")
        ))
    ...
    def test_template_context(self, sample_feed_data: FeedData):
        ...
        assert context["rss_feed_url"] == "https://leblancfg.github.io/deprecations-rss/rss/v1/feed.xml"
        
        # Check deprecations are sorted
        assert len(context["deprecations"]) == 2
        assert context["deprecations"][0].deprecation_date >= context["deprecations"][1].deprecation_date
